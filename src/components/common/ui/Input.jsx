...
const Input = ({ className, variant = 'default', size = 'default', ...props }) => {
  ...
  return (
    <input
      className={`${variantClasses} ${sizeClasses} w-full rounded-md border border-black text-sm transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${className}`}
      {...props}
    />
  );
};
